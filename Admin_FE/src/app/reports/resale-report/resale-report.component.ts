...
export class ResaleReportComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (newObject !== null) {
      ...
      newTrail.append('AuditEntryTypeID', '56');
      newTrail.append('Employee_ID', userObject.employee_ID);
      newTrail.append('Comment', "Visited Resale Report Page");
  ...
  generatePDF() {
    ...
    const tableColumns = ['Date', 'Amount Exchanged', 'Resale Description', 'Evaluation Description'];
    const rowData = this.resaleReports.map(item => [
      item.date,
    ...
    const tableHeight = rowData.length * rowHeight;
    ...
    try {
      ...
      if (newObject !== null) {
        ...
        newTrail.append('AuditEntryTypeID', '56');
        newTrail.append('Employee_ID', userObject.employee_ID);
        newTrail.append('Comment', "Downloaded Resale 's Report");

        this.dataService.GenerateAuditTrail(newTrail).subscribe(response => {
          doc.save('Reseller_Report ' + Date.now() + '.pdf');
        });
      }
      else {
