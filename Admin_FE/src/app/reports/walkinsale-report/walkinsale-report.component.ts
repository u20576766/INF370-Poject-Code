import { Component, OnInit, ElementRef, ViewChild } from '@angular/core';
import { DataService } from 'src/app/services/data.service';
import jsPDF from 'jspdf';
import autoTable from 'jspdf-autotable';
import { SaleReport } from 'src/app/shared/salereport';
import 'jspdf-autotable';
import Swal from 'sweetalert2';
import { ChangeDetectorRef } from '@angular/core';
import * as jspdf from 'jspdf';
import html2canvas from 'html2canvas';
import { Chart } from 'chart.js/auto';


import 'jspdf-autotable';

@Component({
  selector: 'app-walkinsale-report',
  templateUrl: './walkinsale-report.component.html',
  styleUrls: ['./walkinsale-report.component.scss']
})
export class WalkinsaleReportComponent implements OnInit {
  saleReport: SaleReport | undefined;
  Sales: any[] = [];
  @ViewChild('reportContainer') reportContainer!: ElementRef;


  constructor(private dataService: DataService) { }

  toggleDetails(details: any): void {
    details.showDetails = !details.showDetails;
  }

  ngOnInit(): void {
    this.dataService.generateWalkInSaleReport().subscribe((report: SaleReport) => {
      this.saleReport = report;
    });

    let newObject = window.localStorage.getItem("loggedInUser");
    if (newObject !== null) {
      const userObject = JSON.parse(newObject);
      this.fullname = userObject.name + " " + userObject.surname;

      let newTrail = new FormData();
      newTrail.append('AuditEntryTypeID', '52');
      newTrail.append('Employee_ID', userObject.employee_ID);
      newTrail.append('Comment', "Visited Sale Report Page");

      this.dataService.GenerateAuditTrail(newTrail).subscribe(response => {
      });
    }
    else {
      console.log("loggedInUser is null");
      Swal.fire('Error','Error generating the sale report.','error')
    }
  }

  stringDate!: string;
  fullname: string = "";
  img: string = 'data:image/png;base64,'; // Your image data

  downloadResaleReport() {
    const doc = new jsPDF();
    const pageCount = (doc as any).internal.getNumberOfPages();
    // Reset the font style and color for the rest of the document
    doc.setFont('helvetica'); // Reset font style to regular
    doc.setTextColor(0); // Reset text color to black (RGB)

    const date = new Date();
    this.stringDate = date.toString();

    // Add the image at the top left corner
    const imgWidth = 20;
    const imgHeight = 20;
    doc.addImage(this.img, 'PNG', 10, 10, imgWidth, imgHeight);
    let text = "";
    text = "The Book Market\n114 Duxbury Rd\nPretoria, South Africa\n\nWalk-In Sales Report\nReport Date: "
      + date.toDateString() + "\nGenerated By: " + this.fullname;

    const textX = 10 + imgWidth + 10; // 10 pixels spacing between image and text
    const textY = 14; // Adjust this value to position the text vertically
    const maxWidth = 90; // Adjust the maximum width of the text
    const lines = doc.splitTextToSize(text, maxWidth);

    const fontSize = 10; // Adjust the font size
    doc.setFontSize(fontSize); // Set the font size
    doc.text(lines, textX, textY);

    // Calculate the Y position for the table (below the image and text)
    const lineHeight = fontSize * 0.5; // Adjust line height based on font size
    const tableY = Math.max(textY + lines.length * lineHeight); // Adjust spacing
    const textBlockHeight = lines.length * lineHeight;


    // Define table columns

    const Vouchercolumns = ['Voucher Code', 'Total Orders with Voucher', 'Total Discount Amount from Voucher (R)'];
    let Voucherdata: any[] = [];

    if (this.saleReport && this.saleReport.voucherBreakdowns && this.saleReport.voucherBreakdowns.$values) {
      Voucherdata = this.saleReport.voucherBreakdowns.$values.map(voucher => [
        voucher.voucherCode,
        voucher.totalOrdersWithVoucher,
        voucher.totalDiscountAmountFromVoucher
      ]);
    }




    const rowHeight = 10; // Adjust row height as needed
    const tableHeight = Voucherdata.length * rowHeight;
    doc.setFontSize(12); // Set font size to 14
    doc.setFont('Arial', 'bold'); // Set font style to bold
    doc.setTextColor(0, 100, 200); // Set text color to blue (RGB)

    // AutoTable plugin to generate the table
    doc.text('Voucher Breakdown Table', textX - 19, textBlockHeight + 11);
    autoTable(doc, {
      head: [Vouchercolumns],
      body: Voucherdata,
      startY: textY + textBlockHeight + 2
    });




    //datebreakdown
    //datebreakdowns
    const DateBreakdownColumns = ['Date', 'Total Orders on Date', 'Total Revenue on  Date (R)'];
    let DateBreakdownData: any[] = [];

    if (this.saleReport && this.saleReport.dateBreakdowns && this.saleReport.dateBreakdowns.$values) {
      DateBreakdownData = this.saleReport.dateBreakdowns.$values.map(dateBreakdown => [
        dateBreakdown.date,
        dateBreakdown.totalOrdersOnDate,
        dateBreakdown.totalRevenueOnDate
      ]);
    }
    // Calculate the Y position for the table (below the image and text)
    //const lineHeight = fontSize * 0.5; // Adjust line height based on font size
    //const tableY = Math.max(textY + textBlockHeight + 2); // Adjust spacing for Voucherdata

    // Define the row height for the date breakdown table
    const dateRowHeight = 10; // Adjust row height as needed
    const dateTableHeight = DateBreakdownData.length * dateRowHeight;

    // Generate the Voucher table

    autoTable(doc, {

      head: [Vouchercolumns],
      body: Voucherdata,
      startY: tableY
    });

    // Calculate the Y position for the date breakdown table (below the Voucher table)
    const dateTableY = tableY + tableHeight + 14; // Adjust spacing between tables

    // Generate the Date Breakdown table
    doc.text('Date Breakdown Table', textX - 19, dateTableY -2);
    autoTable(doc, {
      head: [DateBreakdownColumns],
      body: DateBreakdownData,
      startY: dateTableY
    });

    let facultyData: any[] = []
    if (this.saleReport && this.saleReport.facultyBreakdowns && this.saleReport.facultyBreakdowns.$values) {
      facultyData = this.saleReport.facultyBreakdowns.$values.map(faculty => [
        faculty.facultyName,
        faculty.totalRevenueFromBooks,
        faculty.totalBooksSold
      ])
    }
    const facultyColumns = ['Faculty Name', 'Total Revenue from Books (R)', 'Total number of books sold']
    const facultyTableY = dateTableY + dateTableHeight + 10; // Adjust spacing between tables
    doc.text('Faculty Breakdown Table', textX - 19, facultyTableY - 4);
    autoTable(doc, {
      head: [facultyColumns],
      body: facultyData,
      startY: facultyTableY
    });





    // For each page, print the page number and the total pages
    for (let i = 1; i <= pageCount; i++) {
      doc.setFontSize(10);
      // Go to page i
      doc.setPage(i);
      var pageSize = doc.internal.pageSize;
      var pageHeight = pageSize.height ? pageSize.height : pageSize.getHeight();
      doc.text('Page ' + String(i) + ' of ' + String(pageCount), doc.internal.pageSize.getWidth() / 2, pageHeight - 8); //data.settings.margin.left if you want it on the left
    //  doc.save('"WalkInSalesReport' + Date.now() + '.pdf');

    }

    let newObject = window.localStorage.getItem("loggedInUser");
    if (newObject !== null) {
      const userObject = JSON.parse(newObject);
      this.fullname = userObject.name + " " + userObject.surname;

      let newTrail = new FormData();
      newTrail.append('AuditEntryTypeID', '52');
      newTrail.append('Employee_ID', userObject.employee_ID);
      newTrail.append('Comment', "Downloaded Walk-In Sales Report");

      this.dataService.GenerateAuditTrail(newTrail).subscribe(response => {
      doc.save('"WalkInSalesReport' + Date.now() + '.pdf');
       });
    }
    else {
      console.log("loggedInUser is null");
      Swal.fire('Error','Error downloading the sales report.Please contact support','error')
    }
  }
}
