...
export class EquipmentReportComponent implements OnInit {
    ...
    forkJoin([equipmentReport$, equipmentTypes$]).subscribe(
      ([reportData, typesData]: any) => {
        ...
        if (newObject !== null) {
          ...
          newTrail.append('AuditEntryTypeID', '54');
          newTrail.append('Employee_ID', userObject.employee_ID);
          newTrail.append('Comment', "Visited Lab Equipment  Report Page");
  ...
  generatePDF() {
    ...
    const tableColumns = ['Name', 'Equipment Type', 'Quantity On Hand'];
    ...
    try {
      ...
      if (newObject !== null) {
        ...
        newTrail.append('AuditEntryTypeID', '54');
        newTrail.append('Employee_ID', userObject.employee_ID);
        newTrail.append('Comment', "Downloaded Lab Equipment Report");

        this.dataService.GenerateAuditTrail(newTrail).subscribe(response => {
          doc.save('Equipment_Report ' + Date.now() + '.pdf');
        });
      }
      else {
        console.log("loggedInUser is null");
