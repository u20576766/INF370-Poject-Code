import { Component } from '@angular/core';
import jsPDF from 'jspdf';
import { DataService } from 'src/app/services/data.service';
import { AuditTrail } from 'src/app/shared/audit-trail';
import autoTable from 'jspdf-autotable';
import { MatNativeDateModule } from '@angular/material/core';
import { MatDatepickerInputEvent, MatDatepickerModule } from '@angular/material/datepicker';
import { MatFormFieldModule } from '@angular/material/form-field';
import { FormControl, FormGroup, Validators } from '@angular/forms';
import Swal from 'sweetalert2';


@Component({
  selector: 'app-audittrail-report',
  templateUrl: './audittrail-report.component.html',
  styleUrls: ['./audittrail-report.component.scss']
})
export class AudittrailReportComponent {

  arrAuditTrails: AuditTrail[] = [];
  stringDate: string = "";

  selectedCategory: string = 'All Categories'; // Default value
  searchQuery: string = '';

  minDate: Date;
  maxDate: Date;
  endDateMin!: Date;

  startDate: string = "";
  endDate: string = "";
  fullname: string = "";

  img: string = 'data:image/png;base64,'; // Your image data


  constructor(private dataService: DataService) {
    // Set the minimum to January 1st 20 years in the past and December 31st a year in the future.
    const currentYear = new Date().getFullYear();
    const currentMonth = new Date().getMonth();
    const currentDate = new Date().getDate();

    this.minDate = new Date(currentYear, 0, 1);
    this.maxDate = new Date(currentYear, currentMonth, currentDate);
  }


  formDateFilter: FormGroup = new FormGroup({
    startDate: new FormControl('', [Validators.required]),
    endDate: new FormControl('', [Validators.required]),
  });

  ngOnInit(): void {
    this.getAuditTrail();

    let newObject = window.localStorage.getItem("loggedInUser");
    if (newObject !== null) {
      const userObject = JSON.parse(newObject);
      this.fullname = userObject.name + " " + userObject.surname;

      let newTrail = new FormData();
      newTrail.append('AuditEntryTypeID', '14');
      newTrail.append('Employee_ID', userObject.employee_ID);
      newTrail.append('Comment', "Visited Audit Trail Page");

      this.dataService.GenerateAuditTrail(newTrail).subscribe(response => {
      });
    }
    else {
      console.log("loggedInUser is null");
    }

    if (this.searchQuery = "") {
      this.getAuditTrail();
    }

  }

  addEvent(event: MatDatepickerInputEvent<Date>) {
    if (event.value) {
      this.endDateMin = event.value;
    }
  }

  applyDateFilter() {
    if (!this.formDateFilter.valid) {
      //trigger your form controls
      Object.values(this.formDateFilter.controls).forEach(control => {
        if (control.invalid) {
          control.markAsDirty();
          control.updateValueAndValidity({ onlySelf: true });
        }

        Swal.fire('Please pick a valid start and end date', '', 'error');
      })
    }
    else {
      this.getFilteredAuditTrail();
    }

  }

  getFilteredAuditTrail() {
    // Create a date object for the selected start and end dates
    const startDateTime = new Date(this.formDateFilter.value.startDate);
    const endDateTime = new Date(this.formDateFilter.value.endDate);

    // Set the time for the start date to 00:00
    startDateTime.setHours(0, 0, 0, 0);

    // Set the time for the end date to 23:59
    endDateTime.setHours(23, 59, 59, 999);

    const options = { timeZone: 'Africa/Johannesburg' }; // Specify the South Africa time zone


    // Convert the modified dates to South African time
    this.startDate = startDateTime.toLocaleString('en-ZA', options);
    this.endDate = endDateTime.toLocaleString('en-ZA', options);

    console.log('Start Date:', this.startDate);
    console.log('End Date:', this.endDate);

    this.dataService.GetFilteredAuditTrails(this.startDate, this.endDate).subscribe(response => {
      this.arrAuditTrails = [];
      let listAuditTrail: any[] = response;
      listAuditTrail.forEach((element) => {
        this.arrAuditTrails.push(element);
      });

    },
      (errorResponse) => {
        Swal.fire(errorResponse.error, 'There are no audit trail records', 'info');
      });
  }

  //Get All the Help Tips
  getAuditTrail() {
    this.arrAuditTrails = [];
    this.dataService.GetAuditTrail().subscribe(result => {
      let listAuditTrail: any[] = result;
      listAuditTrail.forEach((element) => {
        this.arrAuditTrails.push(element);
      });
    });
  }

  //Download audit trail report
  downloadAuditTrailReport() {
    
    const doc = new jsPDF();
   
    const date = new Date();
    this.stringDate = date.toString();
    console.log(this.stringDate);
    console.log('Current date: ' + date.toDateString());

    // Add the image at the top left corner
    const imgWidth = 20;
    const imgHeight = 20;
    doc.addImage(this.img, 'PNG', 10, 10, imgWidth, imgHeight);
    let text = "";
    text = "The Book Market\n114 Duxbury Rd\nPretoria, South Africa\n\nAudit Trail Report\nReport Date: "
      + date.toDateString() + "\nDate Range: " + this.startDate + " - " + this.endDate + "\nGenerated By: " + this.fullname;

    const textX = 10 + imgWidth + 10; // 10 pixels spacing between image and text
    const textY = 10; // Adjust this value to position the text vertically
    const maxWidth = 90; // Adjust the maximum width of the text
    const lines = doc.splitTextToSize(text, maxWidth);

    const fontSize = 10; // Adjust the font size
    doc.setFontSize(fontSize); // Set the font size
    doc.text(lines, textX, textY);

    // Calculate the Y position for the table (below the image and text)
    const lineHeight = fontSize * 1.2; // Adjust line height based on font size
    const tableY = Math.max(textY + lines.length * lineHeight); // Adjust spacing

    autoTable(doc, {
      html: '#report-table',
      startY: maxWidth - 40 // Set the starting Y position for the table
    });

    const pageCount = (doc as any).internal.getNumberOfPages();
    // For each page, print the page number and the total pages
    for (let i = 1; i <= pageCount; i++) {
      doc.setFontSize(10);
      // Go to page i
      doc.setPage(i);
      var pageSize = doc.internal.pageSize;
      var pageHeight = pageSize.height ? pageSize.height : pageSize.getHeight();
      doc.text('Page ' + String(i) + ' of ' + String(pageCount), doc.internal.pageSize.getWidth() / 2, pageHeight - 8); //data.settings.margin.left if you want it on the left
    }

    let newObject = window.localStorage.getItem("loggedInUser");
    if (newObject !== null) {
      const userObject = JSON.parse(newObject);
      const fullname = userObject.name + " " + userObject.surname;

      let newTrail = new FormData();
      newTrail.append('AuditEntryTypeID', '57');
      newTrail.append('Employee_ID', userObject.employee_ID);
      newTrail.append('Comment', "Downloaded Audit Trail Report");

      this.dataService.GenerateAuditTrail(newTrail).subscribe(response => {
        doc.save('AuditTrail ' + Date.now() + '.pdf');
      });
    }
    else {
      console.log("loggedInUser is null");
    }

  }

  searchByCategory(){
      this.dataService.GetFilteredByCategoryAuditTrails(this.selectedCategory, this.searchQuery).subscribe((result) => {
        this.arrAuditTrails = [];
        this.arrAuditTrails = result;
      },
      (errorResponse) => {
        Swal.fire(errorResponse.error, 'There are no audit trail records', 'info');
      });
  }


}
