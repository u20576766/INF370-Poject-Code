...
export class BookReportComponent implements OnInit {
  ...
  filteredBookReportData: BookReportItem[] = [];
  ...
  loadBookReport() {
    this.dataService.generateBookInventoryReport().subscribe(
      (response: BookReport) => {
        ...
        // Initialize filtered data with all books
        this.filteredBookReportData = [...this.bookReportData];
        ...
        if (newObject !== null) {
          ...
          newTrail.append('AuditEntryTypeID', '53');
          newTrail.append('Employee_ID', userObject.employee_ID);
          newTrail.append('Comment', "Visited Book Inventory Report Page");
  ...
  // Apply filters based on min and max base prices
  applyFilters() {
    this.filteredBookReportData = this.bookReportData.filter(book => {
      const basePrice = book.basePrice;
      return (
        (this.minBasePrice === undefined || basePrice >= this.minBasePrice) &&
        (this.maxBasePrice === undefined || basePrice <= this.maxBasePrice)
      );
    });

    // Sort the filtered data by title
    this.sortByTitle();
  }
  ...
  // Sort the data by title in the specified order
  // Sort the data by title in the specified order
sortByTitle() {
  this.filteredBookReportData.sort((a, b) => {
    const titleA = (a.title || '').toUpperCase(); // Use an empty string as a fallback
    const titleB = (b.title || '').toUpperCase(); // Use an empty string as a fallback
    if (titleA < titleB) {
      return this.sortByTitleAscending ? -1 : 1;
    }
    if (titleA > titleB) {
      return this.sortByTitleAscending ? 1 : -1;
    }
  ...
  const stringDate = currentDate.toLocaleDateString();
    ...
    doc.addImage(this.img, 'PNG', 10, 10, imgWidth, imgHeight);
      ...
      let text = "The Book Market\n114 Duxbury Rd\nPretoria, South Africa\n\nBook Inventory Report\nReport Date: " + stringDate  +"\nGenerated By: " + this.fullname;

      // Calculate the maximum text width to fit the page
      ...
      const textY = Math.max(10, imgHeight + 2);
      // Set the font size
      ...
      const tableColumns = ['ISBN', 'Quantity On Hand', 'Title', 'Base Price', 'Price'];
      const tableRows: any[] = [];

      this.filteredBookReportData.forEach((book: BookReportItem) => {
        const rowData = [
          book.isbn,
      ...
      try {
        ...
        if (newObject !== null) {
          ...
          newTrail.append('AuditEntryTypeID', '53');
          newTrail.append('Employee_ID', userObject.employee_ID);
          newTrail.append('Comment', "Downloaded Book Inventory Report");

          this.dataService.GenerateAuditTrail(newTrail).subscribe(response => {
            doc.save('Book_Inventory_Report ' + Date.now() + '.pdf');
          });
        }
        else {
    ...
    showRetrieveErrorAlert() {
      Swal.fire({
        title: 'Error',
        text: 'Failed to retrieve schedule summary data. Try again?',
        icon: 'error',
